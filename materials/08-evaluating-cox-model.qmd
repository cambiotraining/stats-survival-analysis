...
- [Independent observations](05-comparing-two-curves.qmd#sec-independence)
- [Non-informative censoring](05-comparing-two-curves.qmd#sec-non-informative-censoring)
...
## Linearity {#sec-linearity-hospstays-1}
...
## Proportional hazards {#sec-prophaz-hospstays-1}

In a [previous chapter](05-comparing-two-curves.qmd#sec-proportional-hazards-1) the idea of proportional hazards was introduced in the context of the log-rank test.
...
However, examining the survival curves like this is only really helpful for categorical predictors - as we [discussed last chapter](07-cox-regression.qmd#sec-continuous-curves) things get tricky with continuous predictors such as `age`.
...
However, once we've checked the proportional hazards, we could run `coxph(Surv(time, discharged) ~ surgery` just fine - check out the [second exercise from last chapter](07-cox-regression.qmd#sec-exr_cog-vs-logrank) for the proof!
...
In the last chapter, you fitted several Cox regression models to the lung cancer dataset from @sec-exr_lung.
