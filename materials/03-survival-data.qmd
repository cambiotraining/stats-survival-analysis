...
In survival analysis, the outcome variable is **time** (until the event of interest occurs).

Now, we could try to treat that as a continuous response and fit a linear model, but we would run into several issues. We'll start by unpacking those first in this chapter, to explain why we need specialist statistical models.

Then, we'll have a look at some example datasets to focus on how they need to be formatted.
...
This dataset is from a clinical study that tracked 200 patients for up to 30 days following hospital admission, to record whether they were discharged and how long it took.
...
When we plot the observed hospital stay times (or "time-to-discharge") on a histogram, we can see a strong right-skew, i.e., a long tail going in the positive direction.
...
Perhaps the most important and unique thing about time-to-event data is that it will often be censored - meaning that the exact event time is not observed for some individuals, although we still have some information about their follow-up time.
...
::: callout-tip
#### Want more on censoring?

Although we're going to focus on the more common censoring mechanisms here, there are others.

We plan to add some bonus chapters that introduce those types of censoring in time-to-event data which are much less common, in case you come across those in your own research or reading.
