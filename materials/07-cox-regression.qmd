...
In this chapter, we'll explore simple Cox regression models (with a single continuous predictor variable) and how to interpret their output.
...
In this chapter, we'll focus on the third of those points.
...
There is a random variable $T$\$T\$ that represents a subject's true time-to-event (often referred to as "survival time"). We can't always observe $T$ directly because of censoring.
...
## The hazard ratio {#sec_hazard-ratios}
...
In other words, for each additional year of `age`, how much does the "risk" of being discharged from hospital change?
...
In our example here, our hazard ratio is 0.9483.

This tells us that each one-year increase in age is associated with a "risk" of hospital discharge that is 0.9483 times as large (about 5.2% lower) than that of someone one year younger.

So, being younger gets you discharged a tiny bit quicker.

::: {#sec_calculating-hazard-ratios .callout-important icon="false"}
...
## Global significance tests {#sec-global-sig-tests}
...
## Survival curve(s) for a continuous predictor {#sec-continuous-curves}

The standard Kaplan-Meier curves that we've been looking at so far are only applicable for categorical predictors.

If we want to visualise a survival curve for a continuous predictor, we have to do a little more work.

One option is to visualise a set of survival curves at specific quantiles (e.g., 25th, 50th and 75th quantile) of our continuous predictor.

Effectively, we are turning our continuous predictor into a categorical predictor for the purpose of plotting. This does lose us some information, but you might prefer it to not being able to visualise at all!
...
##### Step 1: Find specific percentiles for `age` predictor

```{r}
age_vals <- quantile(hospital_stays$age, probs = c(0.25, 0.5, 0.75), na.rm = TRUE)
```

##### Step 2: Create a new data frame

We need to hold any other predictor variables in our dataset constant, e.g., at their mean.

```{r}
new_data <- data.frame(
  age = age_vals,
  sex = mean(hospital_stays$sex),
  surgery = mean(hospital_stays$surgery),
  weekend_admission = mean(hospital_stays$weekend_admission,
  hospital_type = mean(hospital_stays$hospital_type))
)
```

##### Step 3: Compute survival curves for these values

```{r}
hosp_curve_adjust <- survfit(cox_hosp, newdata = new_data)
```

##### Step 4: Plot the curves

```{r}
ggsurvplot(hosp_curve_adjust, data = hospital_stays)
```

As we might expect, we see that younger patients (red) are getting discharged quicker than older patients (blue).
:::
...
You should be able to just call it as if it's already in your environment, or using `data(lung)`, but if you're having trouble loading it up, you can use the following code:
...
1.  Produce a model with `age` only as a predictor. How much is the hazard increased by a 5-year age increase?
2.  Produce a model with `wtloss` only as a predictor. How much is the hazard increased by a 5lb weight loss?

{{< level 3 >}}

As a bonus:

3.  Do the hazard ratios and/or likelihood ratio test results change for each predictor if you include them in a model simultaneously, versus a separate model for each?
...
Our p-value for the combined model also looks different to the p-values we were getting individually:

- Alone, `age` was marginally significant with a p-value of 0.04
- While `wtloss` was not significant, with a p-value of 0.8
- In the combined model, we have a (marginally?) insignificant result of 0.07

So the short answer to the third question of this exercise is: **yes!** Including both predictors together does change things.

In later chapters, we'll look more at models with multiple predictors. But if you're the type who likes thinking ahead, you can use what you already know about linear modelling to start thinking about *why* this change might've occurred, and whether our two-predictor model might be better than our single-predictor offerings.
::::
:::::::::
::::::::::::

### Cox regression versus log-rank {#sec-exr_cox-vs-logrank}

::::: callout-exercise
{{< level 3 >}}

So far in this chapter, we've used Cox regression to model continuous predictors.

Let's see what happens if we try it out on a categorical predictor instead.

Let's return to the `hospital_stays` dataset; in a [previous chapter](#sec-surgery-log-rank) we used a log-rank test to investigate the effect of surgery.

Recreate this log-rank test, and then compare it to a Cox regression with `surgery` as a predictor.
...
First, we run the log-rank test:

```{r, log rank vs cox regression for surgery}
hospital_stays <- read_csv("data/hospital_stays.csv")

survdiff(Surv(time, discharged) ~ surgery, data = hospital_stays)
```

And now, the Cox regression.

Note that we don't need to do anything different syntax-wise, even though `surgery` is categorical rather than continuous - just as with the `lm` function in linear modelling.

```{r, log rank vs cox regression for surgery 2}
cox_surgery <- coxph(Surv(time, discharged) ~ surgery, data = hospital_stays)  
summary(cox_surgery)
```
:::
...
- We can also extract p-values in multiple ways from a Cox model, with slightly different interpretations; the likelihood ratio test is preferred
:::
