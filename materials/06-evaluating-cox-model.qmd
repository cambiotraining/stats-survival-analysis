...
#| message: false
ggcoxfunctional(cox_hosp, type = "martingale")

ggcoxdiagnostics(cox_hosp, type = "martingale")
```
...
However, once we've checked the proportional hazards, we could run `coxph(Surv(time, discharged) ~ surgery` just fine - check out \[last chapter's exercise\](#sec-exr_cox-vs-logrank) for the proof!
...
The `ggcoxdiagnostics` function from the `survminer` package, which we used earlier for the Martingale residuals plot, can also be used to produce Schoenfeld residuals plots.

All you have to do is tweak the `type` argument.
...
```{r}
ggcoxdiagnostics(lung_age_only, type = "martingale")
ggcoxdiagnostics(lung_wt_only, type = "martingale")
ggcoxdiagnostics(lung_both, type = "martingale")
```
